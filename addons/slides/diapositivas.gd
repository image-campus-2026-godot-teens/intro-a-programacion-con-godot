...
signal slide_changed(new_slide)
...
func cambiar_diapositiva(indice_nuevo):
	...
	if(indice_viejo != indice_nuevo_corregido):
		...
		entrar()
		slide_changed.emit(diapositiva_actual)
...
func actualizar_diapositiva_visible():
	...
	if is_inside_tree() and get_tree().edited_scene_root == self:
		...
		controles_de_presentador().cambiar_a(diapositiva_actual)
...
func accion_secundaria():
	if not intentar_en_diapositiva_activa("accion_secundaria"):
		retroceder()
